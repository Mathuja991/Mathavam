...
// API Base URL එක environment variable එකෙන් ලබාගැනීම
...
const NotificationBell = ({ user }) => {
  ...
  useEffect(() => {
    const fetchNotifications = async () => {
      ...
      try {
        // FIX: Using API_URL environment variable
    ...
    // FIX: Listener function to handle the custom event

    const handleRefreshTrigger = () => {
      // Only fetch if the user is a Parent (childRegNo exists)

      if (childRegNo) {
        console.log(
          "Booking success event detected. Forcing notification refresh."
        );
    ...
    const interval = setInterval(fetchNotifications, 120000); // Polling interval

    // FIX: Listen for the custom event dispatched from the booking form

    window.addEventListener(
      "BOOKING_SUCCESS_NOTIFICATION_TRIGGER",
    ...
    return () => {
      ...
      // FIX: Clean up the custom event listener
  ...
  // Bell icon එක click කළ විට (Mark as Read)
          ...
          <div className="max-h-96 overflow-y-auto divide-y divide-gray-100 dark:divide-slate-700">
            ...
            {notifications.length === 0 ? (
              ...
              <div className="p-6 text-center">
                <FontAwesomeIcon
                  icon={faBell}
                  className="text-3xl text-gray-300 dark:text-slate-600 mb-2"
                />
              ...
              notifications.map((notif) => (
                ...
                >
                  ...
                  <div
                    className={`mt-1 flex-shrink-0 w-5 h-5 rounded-full grid place-items-center ${
                      !notif.isRead
