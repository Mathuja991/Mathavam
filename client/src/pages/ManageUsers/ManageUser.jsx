...
// =====================================================================================
// === වැඩිදියුණු කළ User Management Component එක (වර්ණ, Fonts, Styles සහිතව) ===
// =====================================================================================
...
export default function ManageUser() {
  ...
  // 1. Token එක ලබාගෙන Header එක සෑදීමේ Function එක (වෙනස් කර නැත)
  ...
  // 2. Users ලැයිස්තුව ලබාගැනීමේ Function එක (වෙනස් කර නැත)
  ...
  // Component එක load වූ විට fetchUsers call කිරීම (වෙනස් කර නැත)
  useEffect(() => {
    fetchUsers();
  }, [fetchUsers]);

  // 3. User ව Delete කිරීමේ Function එක (වෙනස් කර නැත)
  ...
  // UserType එක මත පදනම්ව tag එකට වර්ණ ලබා දීම සඳහා නව Function එකක්
  ...
  if (loading && users.length === 0) {
    // Loading State එකේදී වඩාත් ආකර්ශනීය පෙනුමක්
    return (
  ...
  if (error) {
    // Error State එකේදී වඩාත් පැහැදිලි පෙනුමක්
    return (
  ...
  return (
    <div className="max-w-7xl mx-auto p-8 bg-white rounded-3xl shadow-2xl font-['Inter',_sans-serif]">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center pb-6 mb-6 border-b border-gray-100 gap-4">
        {/* Title එකට තද වර්ණයක් සහ stylish font weight එකක් */}
        <h1 className="text-4xl font-extrabold text-indigo-800 tracking-tight">
          Manage Users 👥
        </h1>
        <div className="flex gap-4">
          {/* Add New User Button එකට ආකර්ශනීය වර්ණයක් සහ hover effect එකක් */}
          <button
          ...
          {/* Reset Filters Button එකට මෘදු වර්ණයක් */}
          <button
            onClick={resetFilters}
            className="px-5 py-2 bg-gray-200 text-gray-700 font-medium rounded-xl shadow-md hover:bg-gray-300 transition duration-300"
          >
            Reset Filters 🔄
          </button>
        </div>
      ...
      {/* Filter Section */}
      {/* Filter box එකට ආකර්ශනීය පසුබිම් වර්ණයක් සහ Shadow එකක් */}
      <div className="grid md:grid-cols-3 gap-6 p-6 mb-8 rounded-2xl bg-indigo-50/70 shadow-inner">
        <div className="flex flex-col">
          ...
          {/* Input field එකට Focus Ring එකක් සහ Border Radius එකක් */}
          <input
            type="text"
        ...
        <div className="flex flex-col">
          ...
          <div className="flex gap-3">
            ...
            {/* Sort Direction Button එකට ගැලපෙන වර්ණයක් */}
            <button
              onClick={() =>
                setSortConfig((prev) => ({
                  ...prev,
      ...
      {!loading && filteredAndSortedUsers.length === 0 ? (
        ...
        <div className="overflow-x-auto rounded-2xl shadow-xl border border-gray-200">
          <table className="w-full text-left border-collapse">
            <thead>
              {/* Table Header එකට ආකර්ශනීය gradient එකක් */}
            ...
            <tbody>
              {filteredAndSortedUsers.map((user, idx) => (
                // Table Rows වලට Hover Effect එකක් සහ මෘදු වර්ණ රටාවක්
                <tr
                  key={user._id}
                  className={`border-b border-gray-100 text-gray-800 ${
                    idx % 2 === 0
                      ? "bg-white"
                      : "bg-indigo-50/50" // මෘදු පසුබිම් වර්ණයක්
                ...
                >
                  ...
                  <td className="p-4">
                    {/* User Type එකට වර්ණවත් Badge Style එකක් */}
                    <span
